...
- **Login Persistence (IMPORTANT)**: When saving login state, use `storage_state` parameter instead of `user_data_dir`. The `storage_state` parameter saves only cookies and localStorage, not the entire browser profile. This is the recommended approach for authentication persistence:
  ```python
  # Save cookies after login
  storage_state = await browser.context.storage_state()
  # Load cookies on next run
  browser = Browser(storage_state="path/to/storage_state.json")
  ```
  Note: Do NOT use `user_data_dir` for login persistence, as it copies the entire profile to a temp directory each run, which can cause login state to be lost.
</guidelines>
